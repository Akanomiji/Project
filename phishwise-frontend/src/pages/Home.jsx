...
export default function Home() {
  ...
  const handleScanSubmit = async (e) => {
    ...
    try {
      ...
      if (activeTab === "qr") {
        targetUrl = "http://mock-qr-extracted-link.com/scanned-from-image";
      }

      // ยิงข้อมูลข้ามท่อไปยัง FastAPI Local Server (พอร์ต 8000)
      const response = await axios.post("http://127.0.0.1:8000/api/v1/scan", {
        url: targetUrl,
      });

      // ย้ายหน้าไปยัง /result พร้อมแนบก้อน JSON ผลลัพธ์จริงจากหลังบ้านไปแสดงผล
      navigate("/result", { state: { scanResult: response.data } });
    } catch (error) {
      console.error("เกิดข้อผิดพลาดในการเชื่อมต่อหลังบ้าน:", error);
      alert(
        "⚠️ ไม่สามารถเชื่อมต่อกับระบบประมวลผลหลังบ้านได้\nกรุณาตรวจสอบว่าคุณได้พิมพ์รัน uvicorn main:app --reload ไว้ในอีก Terminal แล้ว",
      );
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <p className="text-lg text-slate-500 mb-10 leading-relaxed max-w-2xl mx-auto">
              ตรวจสอบลิงก์และไฟล์ที่น่าสงสัยด้วยระบบ AI อัจฉริยะ{" "}
            ...
            <form
              ...
              className="bg-white p-2 rounded-2xl shadow-2xl shadow-blue-900/10 border border-slate-100 max-w-2xl mx-auto transform transition-all duration-300"
            >
              ...
              <div className="relative group">
                {activeTab === "url" && (
                  <>
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-36 py-4 bg-white rounded-xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all border border-slate-200 focus:border-blue-500 font-mono text-sm disabled:opacity-60"
                    />
                  </>
                )}
                ...
                {activeTab === "qr" && (
                  <div className="relative">
                    ...
                    <label
                      htmlFor="qr-upload"
                      className={`flex items-center w-full pl-4 pr-36 py-4 bg-white rounded-xl border border-dashed border-slate-300 hover:border-blue-500 hover:bg-blue-50/30 cursor-pointer transition-all group ${isLoading ? "opacity-60 pointer-events-none" : ""}`}
                    >
                      ...
                      <span
                        className={`text-sm truncate max-w-xs ${fileName ? "text-slate-900 font-semibold" : "text-slate-500"}`}
                      >
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="absolute right-2 top-2 bottom-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white px-6 rounded-lg font-bold transition-all shadow-lg shadow-blue-600/30 flex items-center gap-2 hover:translate-x-0.5 text-sm"
                >
                  {isLoading ? (
